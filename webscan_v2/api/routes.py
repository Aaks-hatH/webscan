...
async def report_html(scan_id: str, user=Depends(get_current_user)):
    ...
    return Reporter(_scan_to_result(await _owned(scan_id, str(user["_id"])))).export_html_str()
...
async def report_markdown(scan_id: str, user=Depends(get_current_user)):
    from reporting.reporter import Reporter
    return Reporter(_scan_to_result(await _owned(scan_id, str(user["_id"])))).export_markdown_str()


@router.get("/scans/{scan_id}/report/json")
async def report_json(scan_id: str, user=Depends(get_current_user)):
    from reporting.reporter import Reporter
    return Reporter(_scan_to_result(await _owned(scan_id, str(user["_id"])))).export_json_dict()
